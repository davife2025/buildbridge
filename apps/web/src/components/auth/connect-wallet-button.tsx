...
interface ConnectWalletButtonProps {
  compact?: boolean; // sidebar compact mode: just shows disconnect link
}
...
export function ConnectWalletButton({ compact = false }: ConnectWalletButtonProps) {
  ...
  if (isLoading) {
    return <div className="h-8 w-full animate-pulse rounded-lg bg-gray-200 dark:bg-gray-700" />;
  }

  // ── Compact mode: just a text disconnect link (used in sidebar user card) ──
  if (compact) {
    if (isAuthenticated) {
      return (
        <button
    ...
    return (
      <button
        onClick={connect}
        disabled={isBusy}
        className="btn-brand w-full justify-center text-xs py-2 disabled:cursor-wait"
      >
  ...
  // ── Full mode: used in non-dashboard navbar or pages ──
  if (isAuthenticated && founder) {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <button
          onClick={disconnect}
          className="rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 px-2.5 py-1 text-xs font-medium text-gray-500 shadow-sm transition hover:border-gray-300 dark:hover:border-gray-600 hover:text-gray-700 dark:hover:text-gray-200"
        >
          Disconnect
        </button>
      </div>
