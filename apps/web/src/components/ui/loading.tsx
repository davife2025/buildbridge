interface LoadingSpinnerProps { size?: 'sm' | 'md' | 'lg'; label?: string; fullPage?: boolean; }
const sizes = { sm: 'h-4 w-4', md: 'h-6 w-6', lg: 'h-10 w-10' };

export function LoadingSpinner({ size = 'md', label, fullPage = false }: LoadingSpinnerProps) {
  const spinner = (
    <div className="flex flex-col items-center gap-3">
      <svg className={`${sizes[size]} animate-spin`} style={{ color: '#00C2A8' }} fill="none" viewBox="0 0 24 24" aria-hidden="true">
