...
export function InvestorsContent() {
  ...
  const {
    ...
    isLoadingMatches, isLoadingAll,
    isConnecting, connectedIds, error,
    connectToInvestor,
  } = useInvestors();
  ...
  const TAB_LABELS: { key: Tab; label: string; count?: number }[] = [
    { key: 'matched', label: 'Matched for you', count: matches.length },
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        ...
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-white/40">
            Matched based on your pitch, milestones, and profile.
          </p>
        </div>
        ...
        <div className="mb-6 flex gap-1 rounded-xl border border-white/10 bg-navy-800 p-1">
          {TAB_LABELS.map(({ key, label, count }) => (
            <button
              ...
              onClick={() => setTab(key)}
              className={[
                'flex flex-1 items-center justify-center gap-2 rounded-lg py-2 text-sm transition',
                tab === key
            ...
            {isLoadingMatches ? (
              ...
              <div className="flex flex-col items-center rounded-xl border border-dashed border-white/10 py-16 text-center">
                ...
                <p className="text-sm text-white/20">
                  Complete your pitch and profile to get personalised investor matches.
                </p>
              </div>
            ) : (
              <div className="grid gap-4 sm:grid-cols-2">
                {matches.map((match) => (
                  <InvestorCard
                    ...
                    match={match}
                    isConnected={connectedIds.has(match.investor.id)}
                    isConnecting={isConnecting}
                    onConnect={connectToInvestor}
                  />
                ))}
              </div>
            )}
          ...
          <>
            ...
            <div className="mb-4 flex flex-col gap-3">
              ...
              <InvestorFilters
                ...
                onSectorChange={setSector} onStageChange={setStage}
            ...
            {isLoadingAll ? (
              ...
              <>
                <p className="mb-3 text-sm text-white/30">
                  {filteredAll.length} investor{filteredAll.length !== 1 ? 's' : ''}
                </p>
                <div className="grid gap-4 sm:grid-cols-2">
                  {filteredAll.map((investor) => {
                    ...
                    return (
                      <InvestorCard
                        ...
                        investor={investor}
                        match={match}
                        isConnected={connectedIds.has(investor.id)}
                        isConnecting={isConnecting}
                        onConnect={connectToInvestor}
                      />
                    );
          ...
          <>
            {connections.length === 0 ? (
              <div className="flex flex-col items-center rounded-xl border border-dashed border-white/10 py-16 text-center">
                ...
                <p className="text-sm text-white/20">
                  Request a connection from the Matched or All investors tabs.
                </p>
              </div>
            ) : (
              <div className="space-y-3">
                {connections.map((conn) => (
                  <div
                    key={conn.id}
                    className="flex items-center justify-between rounded-xl border border-white/10 bg-navy-800 p-4"
                  >
                    <div>
                      <p className="font-medium text-white">{conn.investor.name}</p>
                      {conn.investor.firm && (
                        <p className="text-xs text-white/40">{conn.investor.firm}</p>
                      )}
                      {conn.message && (
                        <p className="mt-1 text-xs text-white/30">
                          &ldquo;{conn.message.slice(0, 80)}{conn.message.length > 80 ? '…' : ''}&rdquo;
                        </p>
                      )}
                    </div>
